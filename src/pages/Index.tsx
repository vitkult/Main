...
import { Users, Code, Trophy, Rocket, Heart, ExternalLink, Github, Linkedin, Twitter, Instagram } from "lucide-react";
            ...
            {[
              {
                ...
                description: "Cutting-edge workshops, hackathons, and coding competitions that challenge your skills",
                color: "primary"
              },
              {
                icon: Users,
                title: "Community First",
                description: "A supportive environment where collaboration and knowledge sharing thrive",
                color: "secondary"
              },
              {
                icon: Trophy,
                title: "Competition Ready",
                description: "Regular contests and challenges to keep you sharp and competition-ready",
                color: "accent"
              },
              {
                icon: Rocket,
                title: "Innovation Hub",
                description: "Transform ideas into reality with our project incubation and mentorship programs",
                color: "success"
              }
            ].map((feature, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-foreground mb-3 animate-fade-in">{feature.title}</h3>
                  <p className="text-muted-foreground leading-relaxed animate-fade-in">{feature.description}</p>
                </Card>
              );
            })}
          </div>
        ...
        <div className="max-w-6xl mx-auto text-center">
          <div className="gaming-container p-12 mb-12 animate-slide-in-up">
            ...
            <p className="text-xl text-muted-foreground mb-8 max-w-3xl mx-auto animate-fade-in">
              Join us in shaping the future of technology education. Support innovative minds and gain access to the brightest talent at VIT Bhopal.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in">
              <Button variant="gaming" size="lg" className="group animate-bounce-in" asChild>
                <Link to="/sponsor">
          ...
          <div className="text-center animate-fade-in">
            <h3 className="text-2xl font-bold text-cyber mb-8 animate-fade-in">Trusted By Industry Leaders</h3>
            <div className={`grid grid-cols-1 gap-8 opacity-60 hover:opacity-100 transition-opacity duration-500 animate-fade-in ${["Audly.Ai"].length === 1 ? 'max-w-xs mx-auto' : 'grid-cols-2 md:grid-cols-4'}`}>
              {["Audly.Ai"].map((sponsor, index) => (
                <div
                  key={index}
                  className="gaming-container p-6 hover:scale-105 transition-transform duration-300 animate-slide-in-up"
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div className="text-lg font-bold text-electric animate-fade-in">{sponsor}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-20 px-4 animate-fade-in">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl text-muted-foreground mb-12 animate-fade-in">
            Stay connected with the latest updates, events, and opportunities. Follow us on social media and be part of the VITKULT family!
          </p>
          <div className="grid grid-cols-2 gap-6 mb-12 animate-fade-in">
            {[
              { platform: "LinkedIn", icon: Linkedin, followers: "100+", color: "secondary", link: "https://in.linkedin.com/company/vitkult-vitb" },
              { platform: "Instagram", icon: Instagram, followers: "200+", color: "success", link: "https://www.instagram.com/vitkult.vitb" }
            ].map((social, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="font-bold text-foreground mb-1 animate-fade-in">{social.platform}</h3>
                  <p className="text-sm text-muted-foreground animate-fade-in">{social.followers} followers</p>
                </Card>
              );
            })}
          </div>
          <Button variant="gaming" size="lg" className="group animate-bounce-in" onClick={() => setIsJoinCommunityModalOpen(true)}>
            <Users className="h-5 w-5 group-hover:scale-110 transition-transform duration-300" />
            Join Our Community
            <ExternalLink className="h-4 w-4 group-hover:translate-x-1 transition-transform duration-300" />
          </Button>
          
          <Dialog open={isJoinCommunityModalOpen} onOpenChange={setIsJoinCommunityModalOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>Join Our Community</DialogTitle>
                <DialogDescription>
                  To join our community, please send an email to vitkultclub@vitbhopal.ac.in with your details and interests. Our team will get back to you with further information.
                </DialogDescription>
              </DialogHeader>
              <div className="flex items-center space-x-2">
                <div className="grid flex-1 gap-2">
                  <p className="text-sm text-muted-foreground">
                    Email: <span className="font-semibold">vitkultclub@vitbhopal.ac.in</span>
                  </p>
                </div>
              </div>
      ...
      <footer className="py-12 px-4 border-t border-primary/20 bg-card/50 animate-fade-in">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="flex justify-center space-x-6 mb-6">
            {[
              { icon: Linkedin, link: "https://in.linkedin.com/company/vitkult-vitb" },
              { icon: Instagram, link: "https://www.instagram.com/vitkult.vitb" }
            ].map((social, index) => {
              const Icon = social.icon;
              return (
                <Button
